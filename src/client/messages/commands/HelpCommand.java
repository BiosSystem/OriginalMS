    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splittedLineLineLine) throws Exception,
        ...
        if (splittedLineLineLine[0].equals("!startev")) {
            ...
            if (player.getClient().getChannelServer().eventOn == false) {
                ...
                try {
                    ChannelServer.getInstance(c.getSelectedChannel()).getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(6, c.getSelectedChannel(), "[Event] The event has started in Channel (" + c.getSelectedChannel() + "). Use @event to participate.").getBytes());
                ...
                try {
                    ChannelServer.getInstance(c.getSelectedChannel()).getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(6, c.getSelectedChannel(), "[Event] The event has ended, thank you to everyone who participated.").getBytes());
                } catch (RemoteException e) {
                    ChannelServer.getInstance(c.getSelectedChannel()).reconnectWorld();
                }
            }
            ...
            mc.dropMessage("== BiosMS Commands ==");
