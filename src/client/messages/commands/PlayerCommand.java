...
public class PlayerCommand implements Command {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception, IllegalCommandSyntaxException {
        ...
        if (splitted[0].equalsIgnoreCase("@gm") || splitted[0].equalsIgnoreCase("!gm")) {
            ...
            if (gmUsages.get(c.getPlayer().getId()) != null) {
                ...
                if (System.currentTimeMillis() - lastUse < 60 * 1000 * 2) {
                    mc.dropMessage("You can only send messages to a GM every 2 minutes.");
                    return;
                } else {
                    mc.dropMessage("Sending message...");
                    FilePrinter.printGM("GMSupport.rtf", "Message sent: " + StringUtil.joinStringFrom(splitted, 1) + "\r\nOn: " + sdf.format(Calendar.getInstance().getTime()) + " at " + sdf2.format(Calendar.getInstance().getTime()) + ".\r\nSender: " + player.getName() + " (" + player.getAccountID() + ")");
                    ...
                    mc.dropMessage("Done, please wait for a response.");
                }
            } else {
                mc.dropMessage("Sending message...");
        ...
        if (splitted[0].equalsIgnoreCase("@bugs")) {
        ...
        } else if (splitted[0].equalsIgnoreCase("@comandos") || splitted[0].equalsIgnoreCase("@commands") || splitted[0].equalsIgnoreCase("!comandos") || splitted[0].equalsIgnoreCase("!commands")) {
            mc.dropMessage(" - BiosMS v.62 -");
            for (CommandDefinition cd : getDefinition()) {
                if (!cd.getCommand().equalsIgnoreCase("ajuda") && !cd.getCommand().equalsIgnoreCase("help")) {
            ...
            mc.dropMessage("Done!");
        } else if (splitted[0].equalsIgnoreCase("@grupofix") || splitted[0].equalsIgnoreCase("@partyfix")) {
            player.setParty(null);
            player.dropMessage("Please recreate the party.");
        } else if (splitted[0].equalsIgnoreCase("@evento") || splitted[0].equalsIgnoreCase("@event")) {
            if (player.getClient().getChannelServer().eventOn == true) {
                ...
                mc.dropMessage("There is no active event at the moment.");
            }
        } else if (splitted[0].equalsIgnoreCase("@sairevento") || splitted[0].equalsIgnoreCase("@leaveevent")) {
        ...
        } else if (splitted[0].equalsIgnoreCase("@entrartot") || splitted[0].equalsIgnoreCase("@entertot")) {
            if (player.getMapId() == 240000110) {
                player.changeMap(270000100);
            } else {
                c.getPlayer().dropMessage("You are not at the Leafre station!");
                return;
            }
        } else if (splitted[0].equalsIgnoreCase("@sairtot") || splitted[0].equalsIgnoreCase("@leavetot")) {
            ...
            try {
                ...
                player.dropMessage(" .:: Top 3 Jump Quest Players ::. ");
                int i = 1;
                while (rs.next()) {
                    player.dropMessage(i + ". <Name> " + rs.getString("name") + " / <JQ Points> " + rs.getInt("jqrank"));
                    i++;
                }
            } catch (SQLException ex) {
                Logger.getLogger(PlayerCommand.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else if (splitted[0].equalsIgnoreCase("@checarpersonagem") || splitted[0].equalsIgnoreCase("@checkcharacter")) {
            ...
            player.dropMessage("Occupation: " + player.getOccupation());
        } else if (splitted[0].equalsIgnoreCase("@smega")) {
            if (player.getMeso() >= 10000) {
                player.setSmegaEnabled(!player.getSmegaEnabled());
                String text = (!player.getSmegaEnabled() ? "[Deactivating] Super Megaphones have been disabled." : "[Activating] Super Megaphones have been enabled.");
                mc.dropMessage(text);
                player.gainMeso(-10000, true);
            } else {
                mc.dropMessage("To toggle Super Megaphones, you need 10,000 mesos.");
            }
        }
    }
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{
            new CommandDefinition("commands", "", "Show help list.", 0),
            new CommandDefinition("comandos", "", "Show help list.", 0),
            new CommandDefinition("gm", "<message>", "Send message to online GMs.", 0),
            new CommandDefinition("dispose", "", "Unstuck NPC interaction / action state.", 0),
            new CommandDefinition("grupofix", "", "Fix party creation issues.", 0),
            new CommandDefinition("partyfix", "", "Fix party creation issues.", 0),
            new CommandDefinition("evento", "", "Warp to the current event map.", 0),
            new CommandDefinition("event", "", "Warp to the current event map.", 0),
            new CommandDefinition("sairevento", "", "Leave event and return to previous map.", 0),
            new CommandDefinition("leaveevent", "", "Leave event and return to previous map.", 0),
            new CommandDefinition("smega", "", "Toggle super megaphones (Cost: 10,000 Mesos).", 0),
            new CommandDefinition("bugs", "<bug report>", "Report bugs to admins.", 0),
            new CommandDefinition("entrartot", "", "Enter Temple of Time station.", 0),
            new CommandDefinition("entertot", "", "Enter Temple of Time station.", 0),
            new CommandDefinition("sairtot", "", "Leave Temple of Time station.", 0),
            new CommandDefinition("leavetot", "", "Leave Temple of Time station.", 0),
        };
    }
}
