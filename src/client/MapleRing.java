...
public class MapleRing implements Comparable<MapleRing> {
    ...
    public static int createRing(int itemid, final MapleCharacter partner1, final MapleCharacter partner2, String message) {
        try {
            ...
            partner1.dropMessage("Congratulations! You and " + partner2.getName() + " have successfully purchased a ring!");
            partner1.dropMessage("Please log off and back in if the rings do not work.");
            partner2.dropMessage("Congratulations! " + partner1.getName() + " purchased a ring for you!");
            partner2.dropMessage(partner1.getName() + " sent you: " + message);
            partner2.dropMessage("Please log off and back in if the rings do not work.");
            return 1;
        } catch (SQLException ex) {
            return 0;
        }
    ...
    public static int[] createRing(MapleClient c, int itemid, int chrId, String chrName, int partnerId, String partnername) {
        try {
            ...
            chr.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "You received a ring from " + player.getName() + ". Please log out and log back in if it does not work correctly."));
