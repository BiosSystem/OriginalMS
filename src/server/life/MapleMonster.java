...
//                attacker.gainBiosPoints(configuracoes.BiosPoints_1);
//                attacker.dropOverheadMessage("You gained " + configuracoes.BiosPoints_1 + " [#r#eBiosPoints#k#n]"); // Ganha Leaderpoints 
//           } if ((int) (Math.random() * 300) < 3) {
//                attacker.gainBiosPoints(configuracoes.BiosPoints_2);
//                attacker.dropOverheadMessage("You gained " + configuracoes.BiosPoints_2 + " [#r#eBiosPoints#k#n]"); //Ganha Leaderpoints  
//           } if ((int) (Math.random() * 400) < 3) {
//                attacker.gainBiosPoints(configuracoes.BiosPoints_3);
//                attacker.dropOverheadMessage("You gained " + configuracoes.BiosPoints_3 + " [#r#eBiosPoints#k#n]"); //Ganha Leaderpoints  
//             }
...
//                attacker.dropMessage("[BiosMS CashPQ] You have a total of (" + attacker.getCashPoints() + ") CashPoints."); // CashPoints
//            } else {
            ...
            for (int i = 0; i < mobs.length; i++) {
                if (this.getId() == mobs[i]) {
                    for (MapleCharacter currmap : killer.getMap().getCharacters()) {
                        ...
                        currmap.dropMessage("[BiosMS BPQ] Congratulations, you killed " + getMobName(false) + ".");
                        currmap.dropMessage("[BiosMS BPQ] You now have (" + currmap.getBossPoints() + ") Boss Points.");
                        currmap.dropMessage("[BiosMS BPQ] O monstro seguinte e " + getMobName(true) + ".");
                    }
                    final int x = i;
                    TimerManager.getInstance().schedule(new Runnable() {
                        @Override
                        public void run() {
                            if (MapleMonster.this.getId() == 9400300) {
                                for (MapleCharacter party : killer.getPartyMembers()) {
                                    party.dropMessage("[BiosMS BPQ] You successfully completed the BPQ. ");
                                    party.dropMessage("[BiosMS BPQ] You currently have (" + party.getBossPoints() + ") Boss Points!");
