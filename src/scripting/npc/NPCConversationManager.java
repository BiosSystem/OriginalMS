...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void processGachapon(int[] id, boolean remote) {
        ...
        sendNext("You obtained a #b#t" + itemid + "##k.");
        dispose();
    }
    ...
    public void changeOccupationById(int occ) {
        ...
        getPlayer().sendServerNotice("<BiosMS Occupations> Congratulations " + getPlayer().getName() + ", you are now a " + getPlayer().getOccupation() + ".");
    }
    ...
    public void gainNeckson(int gain) {
        ...
        if (gain > 0) {
            playerMessage(5, "You gained BiosNX (+" + gain + ")");
        } else {
            playerMessage(5, "You lost BiosNX (-" + gain + ")");
        }
    }
    ...
    public int getBiosPoints() {
        return getPlayer().getBiosPoints();
    }

    public void gainBiosPoints(int gain) {
        getPlayer().gainBiosPoints(gain);
        getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(6, "[BiosPoints] You " + (gain > 0 ? "ganhou" : "perdeu") + " (" + gain + ") BiosPoints."));
    }
    ...
    public void gainvotePoints(int gain) {
        ...
        getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(6, "[Vote Points] You " + (gain > 0 ? "ganhou" : "perdeu") + " (" + gain + ") Vote Points."));
    }
    ...
    public void gainPQPoints(int gain) {
        ...
        getPlayer().dropMessage("[BiosMS Quest] Quest successfully completed, you now have (" + getPlayer().getpqPoints() + ") Q.Points!");
    }
