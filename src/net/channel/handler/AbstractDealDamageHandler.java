        ...
        if (!player.isGM()) {
            ...
            if (damageToMonster > elementalMaxDamagePerMonster * multiplyer) {
                ...
                if (damageToMonster > elementalMaxDamagePerMonster * multiplyer) {
                    ...
                    try {
                        player.getClient().getChannelServer().getWorldInterface().broadcastGMMessage("", MaplePacketCreator.serverNotice(5, player.getName() + " has DMG (" + damageToMonster + ") on monster " + monster.getName()).getBytes());
                    } catch (RemoteException ex) {
                        player.getClient().getChannelServer().reconnectWorld();
                    }
