        ...
        switch (itemid) {
            case 2270002: { // Characteristic Stone
                ...
                if (map.getId() != 980010101 && map.getId() != 980010201 && map.getId() != 980010301) {
                    c.getPlayer().dropMessage(1, "This item cannot be used outside of Ariant PQ!");
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
                if (mob.getHp() <= mob.getMaxHp() / 2) {
                    ...
                    c.getPlayer().dropMessage(5, "The monster has too much physical strength, so you cannot capture it.");
                }
                break;
            }
