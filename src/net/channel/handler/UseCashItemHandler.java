                                ...
                                case 1: // Megaphone
                                    if (player.getLevel() >= 10) {
                                        ...
                                        if (c.lastsmegacompare > 60 * 1000 * 5) {
                                            ...
                                            player.dropMessage("Please wait before sending another message.");
                                        }
                                    } else {
                                        player.dropMessage("You need to be level 10 or above to use this item!");
                                    }
                                    break;
                                    ...
                                    if (c.getChannelServer().getMapFactory().getMap(loc.map).getForcedReturnId() == 999999999) {//This doesn't allow tele to GM map, zakum and etc...
                                        if (!victim.isHidden() && !victim.isGM()) {
                                            if (itemId == 5041000) { //viprock
                                                ...
                                                MapleInventoryManipulator.addById(c, itemId, (short) 1, "Teleport Rock error (not found)");
                                                new ServernoticeMapleClientMessageCallback(1, c).dropMessage("Either the player could not be found or you tried to teleport to an invalid location.");
                                                c.getSession().write(MaplePacketCreator.enableActions());
                                            }
                                        } else {
                                            MapleInventoryManipulator.addById(c, itemId, (short) 1, "Teleport Rock error (not found)");
