    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) { // Trade
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    if (!c.getPlayer().hasMerchant() && c.getPlayer().tempHasItems()) {
                        c.getPlayer().dropMessage(1, "Talk to Fredrick to retrieve your stored items.");
                        return;
                    }
                    ...
                    if (c.getPlayer().haveItem(itemId, 1, false, true)) {
                        ...
                        AutobanManager.getInstance().autoban(c, "BiosMS | Merchant Shop: Tried to open a shop without the required item.");
                        return;
                    }
                } else {
                ...
                if (ob instanceof IPlayerInteractionManager && c.getPlayer().getInteraction() == null) {
                    ...
                    if (ips.getShopType() == 1) {
                        ...
                        if (((MaplePlayerShop) ips).isBanned(c.getPlayer().getName())) {
                            c.getPlayer().dropMessage(1, "You have been banned from this shop.");
                            return;
                        }
                    }
            ...
            if (c.getPlayer().getTrade() != null && item != null) {
                if (checkq > 4000) {
                    AutobanManager.getInstance().autoban(c, "BiosMS| PE esta em troca.");
                }
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                if (ivItem != null && ivItem.getQuantity() >= bundles * perBundle) {
                    if (price < 0) {
                        AutobanManager.getInstance().autoban(c, "BiosMS | PE: Added an item with a negative price.");
                        return;
                    }
                    if (bundles <= 0 || perBundle <= 0 || checkquantity > 20000 || checksmquantity < 0 || checkiquantity < 0 || checkiquantity > 20000) {
                        AutobanManager.getInstance().autoban(c, "BiosMS| PE Item de Loja : " + sellItem.getItemId());
                        return;
                    }
            ...
            if (!shop.isOwner(c.getPlayer())) {
                AutobanManager.getInstance().autoban(c, "BiosMS | Tentar remover item de : " + shop.getOwnerName());
                return;
            }
            if (shop != null && shop.isOwner(c.getPlayer())) {
