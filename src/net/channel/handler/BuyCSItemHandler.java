    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (action == 3) {
            ...
            if (!MapleInventoryManipulator.checkSpace(c, itemID, item.getCount(), "")) {
                c.getPlayer().dropMessage(1, "Your inventory is full, please check!");
                updateInformation(c);
                return;
            }
            if (useNX != 2 && item.getDonor() == 1) {
                c.getPlayer().dropMessage(1, "Purchasable only with BiosPoints!");
                updateInformation(c);
                return;
            }
            if (itemID == 5200000) {
            ...
            if (checkBirthday(c, slea.readInt())) {
                ...
                if (victim != null) {
                    ...
                    if (item.getId() == 5220000) {
                        c.getPlayer().dropMessage(1, "This item is not available as a gift.");
                        updateInformation(c);
                        return;
                    }
                    ...
                    if (item.getId() >= 5000000 && item.getId() <= 5000370) {
                        ...
                        c.getPlayer().dropMessage(1, "Gift sent successfully!");
            ...
            if (partnerChar == null) {
                c.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(1, "The specified partner could not be found.\\r\\nPlease check if your partner is online and in the same Channel."));
            } else if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).findById(1112000) != null) {
                c.getPlayer().dropMessage(1, "You already have a ring with someone else!");
                return;
            ...
            if (item != null && c.getPlayer().getMeso() >= item.getPrice()) {
                if (MapleItemInformationProvider.getInstance().isQuestItem(item.getId())) {
                    ...
                    c.getSession().write(MaplePacketCreator.enableActions());
                    c.getPlayer().dropMessage(1, "You cannot buy items with 0 mesos!");
                    return;
                }
            }
        }
